/*
*   Nombre del programa: diccionario.c
*   Autor: Marco Antonio Esquivel Basaldua
*   Fecha: 16/09/2019
*
*   Descripcion del programa:
*       Busca una palabra solicitada por el usuario dentro del texto de Don Quijote
*       dentro de las primeras n palabras seleccionadas por el usuario.
...
int findWord(FILE*,int,char[][N+1],char[]);  // Busca una panlabra solicitada en un arreglo bidimensional de palabras. Si la encuentra regresa 1, si no, regresa 0
...
int main( int argc , char* argv[] ){
    ...
    int sel = 0;
    while(sel != 2){
        printf("Menú Selecciona una opción:\n1) Buscar palabra\n2) Salir\n");
        scanf("%d",&sel);

        if(sel == 1){
            int n;
            printf("Ingresa el número de palabras donde se realizará la búsqueda:\n");
            scanf("%d",&n);
            ...
            int f = findWord(fin,n,palabras,toFind);


            if(f == 1) printf("\nLa palabra buscada fue encontrada\n\n\n");
            else printf("\nLa palabra buscada NO fue encontrada\n\n\n");
        }
        else if(sel != 2) printf("Selección inválida\n\n\n");
    }

    printf("Se ha salido de la aplicación...\n");
...
int findWord(FILE* fin,int n, char palabras[][N+1],char toFind[]){
/*
*   Inputs:
*       - FILE* fin: Archivo de texto a leer
*       - int n: numero total de palabras a leer
*       - char palabras[][N+1]: arreglo con las palabras del texto, las palabras se leen por filas
*       - char toFind: palabra solicitada para buscar en el texto
*
*   Outputs:
*       - int: regresa 1 si se encontro la palabra solisitada
*              regresa 0 si no
*/
    ...
    for(int i=0; i<m; i++){
        int comp = strcmp(palabras[i],toFind);       // Comparacion de una palabra con la de interés
        if(comp == 0) f = 1;
    }
